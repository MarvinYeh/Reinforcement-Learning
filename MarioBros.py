...
with tf.variable_scope('eval-net'):
    ...
    conv1 = tf.layers.conv2d(inputs=image, filters=4, kernel_size=5,activation=tf.nn.relu) #( ,220,252,16)
    conv2 = tf.layers.conv2d(conv1, 8,5,1,activation=tf.nn.relu) #(216,248,32)
    flat = tf.reshape(conv2,[-1,216*248*8])
    hidden1 = tf.layers.dense(flat,10,activation=tf.nn.relu)
    output = tf.layers.dense(hidden1,6,activation=None)
    loss =tf.reduce_mean(tf.squared_difference(tfy,output))
    # tf.summary.scalar('loss', loss)
    # tf.summary.histogram('eval_out', output)
    # tf.summary.histogram('eval_dense',hidden1)
...
with tf.variable_scope('target-net'):
    ...
    conv1_t = tf.layers.conv2d(inputs=image_t, filters=4, kernel_size=5,activation=tf.nn.relu) #( ,220,252,16)
    conv2_t = tf.layers.conv2d(conv1_t, 8,5,1,activation=tf.nn.relu) #(216,248,32)
    flat_t = tf.reshape(conv2_t,[-1,216*248*8])
    hidden1_t = tf.layers.dense(flat_t,10,activation=tf.nn.relu)
